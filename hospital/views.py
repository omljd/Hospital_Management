...
def Add_Doctor(request):
    ...
    if request.method=='POST':
        ...
        try:
            Doctor.objects.create(name=n,mobile=c,spacial=s)
            error='no'
        except:
            error='yes'
    d={'error':error}
...
def Delete_Appointment(request, ap_id):
    if request.user.is_staff:  # Ensure only staff can delete
        appointment = get_object_or_404(Appointment, id=ap_id)
        appointment.delete()
